...
#define nRF24_WAIT_TIMEOUT         (uint32_t)0x000FFFFF	// Timeout counter (depends on the CPU speed). Used for not stuck waiting for IRQ
...
#define nRF24_MASK_INTERRUPT       (uint8_t)0x70 // Mask RX_DR, TX_DS and MAX_RT interrupts
...
// Transceiver state
enum {
	nRF24_STATE_RX = (uint8_t)0x01, // PRX
	nRF24_STATE_TX = (uint8_t)0x00  // PTX
};
...
typedef enum {											// Result of packet transmission
...
extern uint8_t nRF24_STATE;
extern volatile uint8_t nRF24_ADDR[];
