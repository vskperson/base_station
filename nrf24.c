...
static void nRF24_ReadMBReg(uint8_t reg, uint8_t *pBuf, uint8_t count)
{
	nRF24_CSN_L();
	nRF24_LL_RW(reg);
	while (count--) {*pBuf++ = nRF24_LL_RW((uint8_t)0xFF);}
	nRF24_CSN_H();
}
...
void nRF24_IntDisable() {
	uint8_t reg;

	reg  = nRF24_ReadReg(nRF24_REG_CONFIG);
	reg &= ~nRF24_MASK_INTERRUPT;
	reg |= (0b00110000 & nRF24_MASK_INTERRUPT);		//only RX_DR interrupt is active
	nRF24_WriteReg(nRF24_REG_CONFIG, reg);
}
...
volatile void nRF24_WritePayload(uint8_t *pBuf, uint8_t length) {
...
volatile nRF24_TXResult nRF24_TransmitPacket(uint8_t *pBuf, uint8_t length)
{
...
//	UART_TXDelay;											// Check the flags in STATUS register
...
//	UART_TXDelay;
	...
	if (status & nRF24_FLAG_MAX_RT){return nRF24_TX_MAXRT;}		// Auto retransmit counter exceeds the programmed maximum limit (FIFO is not removed)
	if (status & nRF24_FLAG_TX_DS){return nRF24_TX_SUCCESS;}	// Successful transmission
	nRF24_FlushTX();											// Some banana happens, a payload remains in the TX FIFO, flush it
	return nRF24_TX_ERROR;
}

void nRF24_ReConfig_TX()
{
	nRF24_CE_L();
...
//	nRF24_SetAddrWidth(3);						// Set address width, its common for all pipes (RX and TX)
	...
	nRF24_SetAutoRetr(nRF24_ARD_2500us, 0);		// 5 auto retries with pause of 2500s in between
